...
###############################
def save_credit():
    global total_inserted # Stačí nám jen total_inserted
    with open(credit_file, "w") as f:
        # Credit je VŽDY 0, total_inserted je skutečně vložená částka
        json.dump({"credit": 0, "total_inserted": total_inserted}, f)
...
def load_credit():
    ...
    try:
        with open(credit_file, "r") as f:
            data = json.load(f)
            # Načítáme POUZE total_inserted. Credit ignorujeme, je irelevantní.
...
def bill(value):
    ...
    # 🔥 KLÍČOVÁ ZMĚNA: Před přidáním peněz ZNOVU načti aktuální stav
    # Zjistíme, jestli jiný script neresetoval soubor credit.json na 0/0.
    load_credit() 
    
    # Pokud load_credit nastavilo total_inserted na 0, začínáme od nuly.
    total_inserted += value
    
    # Původní tisk se zjednoduší
    print(f"💰 Peníze přidány: +{value} Kč. Celkem vloženo: {total_inserted} Kč")
    
    save_credit() # Uložíme nový součet
    ...
    while True:
        if ser.in_waiting > 0:
            ...
            # Potvrzení přijetí / odmítnutí
                ...
                if escrow_value > 0:
                    print(f"✅ Přijato {escrow_value} Kč")
                    bill(escrow_value)
                    escrow_value = 0
            elif byte_in == 173:
