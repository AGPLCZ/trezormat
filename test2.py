...
#
# Tento skript navíc automaticky vyhledá port, ke kterému je NV10 připojeno (pokud se to nepodaří,
# můžete fallbackem použít ručně zadaný port). Vyhledává například podle klíčového slova "FTDI",
# ale můžete si přidat vlastní podmínky. Pokud je v systému více zařízení, je dobré si udělat
# lepší logiku nebo vypsat nabídku pro ruční výběr.
...
def find_nv10_port(keyword="FTDI"):
    ...
    Vrátí např. '/dev/ttyUSB0' nebo None, pokud se nic nenašlo.
    Podmínku můžete rozšířit o vendor_id, product_id atd.
    """
    ports = list(serial.tools.list_ports.comports())
    for p in ports:
        # Převedeme None na prázdný řetězec "" pomocí funkce str() nebo ternárního výrazu
        desc = p.description if p.description else ""
        manu = p.manufacturer if p.manufacturer else ""
        name_ = p.name if p.name else ""

        # Hledáme klíčové slovo ve všech polích, která nejsou None
